...
const streamObserver = new MutationObserver((mutations) => {
    ...
    if (
        ...
        mutations[1].addedNodes.length === 0
    ) {
        return
    }
    ...
    const oldButton =
        removed.tagName === 'BUTTON'
            ? removed
            : (removed.querySelector('button') as Element)
    const newButton =
        added.tagName === 'BUTTON'
            ? added
            : (added.querySelector('button') as Element)
    ...
    if (oldButtonTestId == 'send-button' && newButtonTestId == 'stop-button') {
        ...
        oldButtonTestId == 'stop-button' &&
        newButtonTestId == 'send-button'
    ) {
...
const streamObserverStart = () => {
    const [button] = querySendStopSpeechButton()
    streamObserver.observe(button.parentElement?.parentElement as Node, {
        childList: true,
        subtree: true,
    })
}
